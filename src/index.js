...
class JestWatchTogglePlugin {
	constructor({ config } = {}) {
		if (!config) {
			errorOut(`Missing plugin configuration. Are you sure you’re using ${chalk.bold.red('Jest 23.3+')}?`)
		}

		this.setting = getConfigValue(config, 'setting')

		if (!this.setting) {
			errorOut(`${this.constructor.name} needs at least a ${chalk.bold.red('setting')} configuration parameter`)
		}

		const defaults = DEFAULT_CONFIG[this.setting] || {}
		this.key = getConfigValue(config, 'key', defaults) || this.setting[0].toLowerCase()
		this.prompt = getConfigValue(config, 'prompt', defaults) || `turn %ONOFF% ${this.setting}`
	}
...
function errorOut(message) {
	const comment = `${chalk.bold.red('Configuration documentation')}: ${chalk.underline(
		'https://github.com/repobuddy/jest-watch-toggle-config-2#readme',
	)}
  `.trim()
